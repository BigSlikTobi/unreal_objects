...
CMD ["sh", "-c", "test -n \"$RULE_ENGINE_UPSTREAM\" && test -n \"$DECISION_CENTER_UPSTREAM\" && test -n \"$INTERNAL_API_KEY\" && DNS_RESOLVER=$(awk '/^nameserver/{addr=$2; if(addr ~ /:/){addr=\"[\"addr\"]\"}; print addr; exit}' /etc/resolv.conf || echo '8.8.8.8') && sed -e \"s|__PORT__|${PORT:-8080}|g\" -e \"s|__RULE_ENGINE_UPSTREAM__|${RULE_ENGINE_UPSTREAM}|g\" -e \"s|__DECISION_CENTER_UPSTREAM__|${DECISION_CENTER_UPSTREAM}|g\" -e \"s|__INTERNAL_API_KEY__|${INTERNAL_API_KEY}|g\" -e \"s|__DNS_RESOLVER__|${DNS_RESOLVER}|g\" /etc/nginx/templates/default.conf.template > /etc/nginx/conf.d/default.conf && exec nginx -g 'daemon off;'"]
